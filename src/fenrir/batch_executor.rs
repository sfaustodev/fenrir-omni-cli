...
impl BatchExecutor {
    ...
    async fn execute_sequential(&self, job: &BatchJob) -> Result<Vec<ToolExecutionResult>, String> {
        ...
        // Create progress bar with big green rotating spinner
        let pb = ProgressBar::new(job.tools.len() as u64);
        pb.set_style(
        ...
        pb.set_message("Starting sequential execution...");

        for (i, tool_name) in job.tools.iter().enumerate() {
            pb.set_message(format!("🔧 Executing {} (sequential)", tool_name));
        ...
        pb.finish_with_message("Sequential execution completed!");
        Ok(results)
    }
    ...
    async fn execute_parallel(&self, job: &BatchJob) -> Result<Vec<ToolExecutionResult>, String> {
        ...
        // Clone job data to avoid lifetime issues
        ...
        let max_parallel = job.config.max_parallel;

        // Create progress bar with big green rotating spinner
        let pb = Arc::new(ProgressBar::new(tools.len() as u64));
        pb.set_style(
            ProgressStyle::default_bar()
                .template("{spinner:.green} [{elapsed_precise}] [{bar:40.cyan/blue}] {pos:>7}/{len:7} {msg}")
                .unwrap()
        ...
        pb.set_message("Starting parallel execution...");
        ...
        for tool_name in &tools {
            ...
            let timeout = timeout;
            let continue_on_failure = continue_on_failure;
            let semaphore = Arc::clone(&semaphore);
            let engine = Arc::clone(&engine);
            let pb_task = Arc::clone(&pb_clone);

            let handle = tokio::spawn(async move {
                let _permit = semaphore.acquire().await.unwrap();

                pb_task.set_message(format!("🔧 Executing {} (parallel)", tool_name));
        ...
        for handle in handles {
            match handle.await {
                ...
                Ok(Err(e)) => {
                    if !continue_on_failure {
                        pb.finish_with_message("Parallel execution failed!");
                        return Err(e);
                    }
                    // Create error result
        ...
        pb.finish_with_message("Parallel execution completed!");
        Ok(results)
    }
