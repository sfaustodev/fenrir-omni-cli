...
fn show_rules(config: &Config) {
    ...
    println!(
        "      {}",
        "Bloqueia operações envolvendo conteúdo ilegal".dimmed()
    );

    ui::list_bool("Anti-vazamento sensível", policies.anti_sensitive_leaks);
    println!(
        "      {}",
        "Previne exposição de credenciais e dados pessoais".dimmed()
    );
    ...
    println!(
        "      {}",
        "Registra todas as operações para análise".dimmed()
    );

    ui::list_bool("Proteger infra crítica", policies.protect_critical_infra);
    println!(
        "      {}",
        "Bloqueia ações em sistemas críticos conhecidos".dimmed()
    );
    ...
    ui::list_item(
        "Timeout padrão",
        &format!("{}s", config.scan.default_timeout),
    );
...
fn count_active_policies(policies: &crate::config::ContentPolicies) -> usize {
    let mut count = 0;
    if policies.anti_pedophilia {
        count += 1;
    }
    if policies.anti_sensitive_leaks {
        count += 1;
    }
    if !policies.allow_aggressive_pentest {
        count += 1;
    } // Invertido: desativado é mais seguro
    if policies.respect_robots_txt {
        count += 1;
    }
    if policies.audit_logging {
        count += 1;
    }
    if policies.protect_critical_infra {
        count += 1;
    }
    count
}
